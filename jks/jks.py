    ...
    def loads(cls, data, password):
        ...
        for i in xrange(entry_count):
            ...
            if tag == 1:  # private key
                ...
                elif file_type == 'jceks':
                    if algo_id == SUN_JKS_ALGO_ID:
                        plaintext = _sun_jks_pkey_decrypt(encrypted_private_key, password)
                    elif algo_id == SUN_JCE_ALGO_ID:
                        salt = asn1_data[0][0][1][
                            0].asOctets()  # see section A.3: PBES1 and definitions of AlgorithmIdentifier and PBEParameter in RFC 2898
                ...
                if not plaintext:
                    raise ValueError("plaintext variable not set")
...
def _sun_jks_pkey_decrypt(data, password):
    ...
    password = ''.join([b'\0' + c.encode('latin-1') for c in
                        password])  # the JKS algorithm uses a regular Java UTF16-BE string for the password, so insert 0 bytes
...
def _sun_jce_pkey_decrypt(data, password, salt, iteration_count):
    ...
    des3 = DES3.new(key, DES3.MODE_CBC,
                    IV=iv)  # apparently uses EDE, although there's no apparent way to tell it what to use ...
...
def _sun_jce_derive_cipher_key_iv(password, salt, iteration_count):
    ...
    """
    if len(salt) != 8:
        raise ValueError("Expected 8-byte salt for JCE private key encryption algorithm (OID %s), found %d bytes" % (
    ...
    for i in xrange(2):
        ...
        for k in xrange(iteration_count):
...
def _strip_pkcs5_padding(m):
    ...
    elif m[-last_byte:] != chr(last_byte) * last_byte:
        raise ValueError("Unable to strip PKCS5 padding: invalid padding found")
